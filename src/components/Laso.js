...
//kiểm tra trùng sao
  ...
  async function lapDiaBan(){
    ...
    ThapNhiCung[i].tamhop=[...ThapNhiCung[i].sao.map(a =>a.id), ...ThapNhiCung[(i+4)%12].sao.map(a =>a.id), ...ThapNhiCung[(i+8)%12].sao.map(a =>a.id), ...ThapNhiCung[(i+6)%12].sao.map(a =>a.id)]
    //render các cung
    ReactDOM.render(
    <>
